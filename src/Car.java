...
if getTotalDistanceTravelled()>999{
    setCondition("broken");
        }
    }
    ...
    public void addGasToTank(double amount) {
        ...
        //TODO: implement the method
        if (0 < amount ){
            if (TANK_CAPACITY - this.getFuel() > amount) {
                setFuel((int)(getFuel()+ amount));
            }
        }
    }
    ...
    public void drive(int speed, int time) {
        ...
        //TODO: implement the method
        if (getCondition()=="working"){
            distance = speed*time;
            if (distance>0 & distance<= getDistanceUntilEmpty()){
                totalDistanceTravelled=(getTotalDistanceTravelled()+ distance);
                fuel= (getFuel()- (distance/LITRES_PER_100KM*100));
            }
        }
    }
...
// Get speed of the car
    ...
    public double distanceMeter (){
        for(double i= 0;i==500;i++);//the reserve is 5
        System.out.println("distance you have driven is"+double i);// i am printing distance you have driven + double 1(the double that I used in my for loop
    }
